...
| *(endpoint)* `GET /_admin[?session=][&all=1]` | — | The `/_status` snapshot rendered as HTML for humans (the "proxy admin page"): dark single-page table — warmth (🔥 + time left / ❄️ / ∅), title+sid+cwd+model, hold (until HH:MM, pings n/expected — expected = hold duration ÷ ttl, the 24 cap is only the safety bound), pingable/awaiting-auth, per-session cost, refusals; header has flags, totals and a since-restart delta. Server-rendered, zero JS, `html.escape` on everything (titles are model output), meta-refresh 10s. Same lab-grade auth posture as the other endpoints (none — localhost). |
