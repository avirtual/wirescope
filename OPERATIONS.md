...
The capture fixtures under `fixtures/muse/` came from `muse exec --provider meta --base-url http://127.0.0.1:<port>/agent/probe` against a local stub; the live A/B against Meta's real endpoint ran 2026-09-22 on a subscription account (see CHANGELOG / CLAUDE.md feature row).

**Seat-launch facts for a wrapper (all read off muse 1.3.0-R3401.1, 2026-09-22, by running the binary, not from --help):**
- `--session-id <UUID>` exists ONLY on `muse exec`. The interactive TUI rejects it (`unexpected argument '--session-id'`) and has no `--session-name` either; the TUI mints its own UUIDv7 and the wrapper learns it after the fact from the runtime registry `~/.local/share/muse/runtime/muse/sessions/<sid>.json` (written at launch, carries `session_id`, `session_name`, `pid`) or `~/.local/share/muse/session-index.db` (`sessions` table). The transcript path is always `$XDG_DATA_HOME/muse/sessions/YYYY/MM/DD/<sid>/session.jsonl`.
- `--permission-profile <ID>` accepts four built-ins (`:read-only`, `:ask-me`, `:auto-review`, `:unrestricted`) plus user-named profiles from `settings.json`. `:read-only` = filesystem `:minimal`/`:root`/`:workspace_roots` all `read`, network `restricted`, approval `on_request`, reviewer `human` — so headless it HANGS on the first bash call waiting for an approval that never comes (live: `approval_wait.effect.started` then `cancelled` on SIGTERM). The flag is mutually exclusive with `--yolo` AND `--approval-mode`. The headless read-only cap is therefore a user profile extending the built-in:
  `{"schema_version":1,"permissions":{"schema_version":1,"profiles":{"reviewer":{"extends":":read-only","approval":"allow_all","reviewer":"none"}}}}` in the seat's `settings.json`, launched with `--permission-profile reviewer`. Live-verified: `cat` runs, `echo > file` fails inside the tool (`Operation not permitted`), `write_file` returns `tool policy denied filesystem write`; the run completes. The four dimensions are `filesystem`, `local_command_network`, `approval`, `reviewer`; the inheritance key is `extends` (not `parent`/`inherits`); a parentless profile must define all four; nothing may extend `:unrestricted`.
- Transcript shape: one JSON record per line, `{payload_type, payload, record_type, sequence, stream:{kind:"session",id}}`; the assistant text is `payload_type:"runtime.session"` with `payload.kind:"run"` and `payload.event.kind:"assistant_message_committed"` (`.text`), usage is `event.kind:"model_completed"`, the prompt is `runtime.user_intent.accepted`. Some lines are wrapper frames (`retained_frame:"session_permission_transaction"` with `children[].record_json` as an embedded JSON string; `omitted_live_only` markers) — a parser must accept both. `muse exec --json` streams the same records on stdout.
...
`digest_session.py` — mechanical (no-LLM) session digest for handoff or skim: `python3 digest_session.py <transcript.jsonl | capture_dir> [-o out.md] [--last N] [--thinking] [--full] [--utc]`. Renders one `## ` heading per real user prompt with a chronological one-line-per-event body: the model's own text trimmed to 300 chars, tool calls as `Bash: <description>` / `Edit /path (+N/-M chars)` / `Write /path (N lines)` (never the payload), tool failures and large results flagged, and the clodex intent traffic parsed into `→ dm to X` / `← dm from X` / `✔ ticket tNNN closed` / `exec name → result` lines; the header counts tools, peers and tickets over the whole session. Compresses ~2,000:1 (47 MB transcript → 875 KB full, 9 KB with `--last 6`). On a capture dir it walks the main-line requests in order and stamps each message with its debut request, so it recovers the full history across compacts and real timestamps (a request carries no per-message time); the walk tolerates the settled-history rewrites that are not edits (reminder blocks dropped, block→string, marker moves, tool_result folds, trailing tail hints, tool-less side-calls), because keying on raw bytes read 2,159 of 4,820 turns as compacts. `--last N` is the handoff mode: header over everything, body for the last N prompts only.

`analyze_busts.py` — corpus-wide cache-bust ledger: `python3 analyze_busts.py [--logs DIR] [--since DAYS] [--session ID] [--top N] [--json rows.json] [--min-usd X]`. Runs `report.bust_series` (the /_bust engine) over every session in a capture root and aggregates by fault owner, class, recurring pattern (locus label + old→new snippet with digits blanked, so a date rollover on two days is one row), agent, session, and by what the PROXY did on the busted request (the transform logs captured next to `body`: marker gate mode, pin reason, compact-strip firing). Pricing is marginal, `(paid rate − read rate)` at the receipt's own TTL, so a class's line reads as "what a warm run would have saved". Read the docstring for what a row is and what it cannot see (cold boots are analyze_prefix.py's job). First run (2026-09-09, 3 days) found three classifier defects and one live proxy defect, all fixed the same day; treat a class that dominates the ledger as a question about the classifier first and the wire second. Since v0.6.65 it ends with a keep-warm ping ledger (`--no-pings` to skip, `--pings-json rows.json` to dump): pings per seat priced as cached read / uncached tail / write, dirty pings (a write, or >4k uncached) named, and ping LOOPS (≥5 BILLED pings ≤120 s apart, no organic turn between) listed with duration, cost and stash TTL — a loop means a hold is pinging a prefix whose TTL is inside the ping margin (the 5m compact-stash case, 152 pings/$7.53 in one night, 2026-09-09). Since 2026-09-22 a run of ≥3 consecutive FAILED pings at the same spacing is reported apart as a ping BURST: identical on the clock, opposite meaning — a burst is the keeper's decline path under an upstream outage, costs $0, and what you read off it is how many attempts remained when the TTL ran out (three seats × five 529s over a 240 s window inside a 300 s margin that morning; one expired prefix cost $2.93 to re-write). Keeping both in one table would file a keeper that retries FASTER near expiry as a new loop, i.e. report the hardening as the defect. Pings are recognised by `summary.keepwarm` on v0.6.65+ captures and by the wire shape (tools + `max_tokens:1`) on older ones, so the ledger prices history.

`analyze_waits.py` — wait-polling ledger: `python3 analyze_waits.py [--logs DIR] [--since YYYY-MM-DD[THH:MM]] [--only SUBSTR] [--text-noise N] [--out rows.json] [--print-top N]`. Finds runs of ≥3 consecutive tool-only assistant turns repeating a near-identical read-only wait shape (date, sleep, ps/pgrep, jobs, git status, curl, or a byte-identical Read/ls/cat of the same file) with no edit and no incoming input between, prices each run from the receipts in its message range (paired by stem), and reports prevalence, run lengths, shapes, what the run was waiting for, and how often the seat used the correct alternatives (`run_in_background`, `[agent:exec clodex-monitor]`, a single blocking in-shell wait). Test/build commands are classed as retry loops and excluded. `--text-noise 400` is the honest cost view (a short "still waiting" sentence does not break a run); the default 40 is the strict view. `--since` keys on a session's FIRST capture, so a re-run after a prompt change measures only seats spawned after it. First run 2026-09-09: 6/470 hand sessions (1.3%), $28, 53M tokens re-read, every run waiting on the seat's OWN run-tests digest; clodex moved the "end your turn" instruction directly under the exec line at 11:54. Re-run 2026-09-16 (`--since 2026-09-09T11:54`): the placement did NOT hold — 5/232 hand sessions, 25 runs, $63.18, i.e. $0.060 -> $0.272 per hand session. **Read the PER-CHANNEL split, not the total**: attribute each run to the nearest preceding async launch (background-Bash result string, run-tests exec, Agent launch) within ~120 messages of its first poll turn, since a prompt line under ONE launch site leaves the others untouched — the fix's own channel stayed flat ($0.041 -> $0.047/session) while waits on a spawned subagent went $0.0003 -> $0.176 and on a `run_in_background` Bash $0.012 -> $0.050. Clodex answered with hooks (t935, live at the 2026-09-16 16:06:45 +03:00 relaunch): PreToolUse denies the 3rd identical consecutive Bash call from a ticket hand, PostToolUse appends an end-your-turn line after an Agent launch or a background Bash. It counts Bash only, so Read-tool re-reads and cat/tail alternation on one path are a known uncovered shape.
