...
| **Pull** | `/_status[?session=<id>][&all=1]` | GET | free | Durable JSON: sessions, titles/agent, model, cwd, per-session + global cost, turn counts, context size (`context.input_tokens` = the wire-measured input-side token count of the last turn — cache_read + cache_write + uncached input, the same figure `/_session` shows; plus `turns_in_context` / `n_messages`), warmth, hold, refusal events, and **`busts`** — the per-session **real-bust** rollup (a *real* bust = a turn whose write landed UPSTREAM of the prior cache end, previously-cached tokens invalidated + re-paid at the write premium, NOT the normal tail-append write every turn pays to cache its own fresh content). `null` when the session never had one. Shape: `{total, actionable (total − lapse), by_class:{tools, system, preamble, conversation, lapse}, classes:[{class, count, fault, fix_hint} biggest-first], last_bust:{class, ts, write_tokens, fault, fix_hint}}`. The five **classes** are structural (divergences land at cache boundaries, not random offsets): `tools` (total bust — tools is first in cache order), `system` (near-total — model/prompt swap), `preamble` (`messages[0]` = claudeMd/userEmail/currentDate/scratchpad changed → history re-writes, tools+system survive), `conversation` (rare mid-history edit), `lapse` (cache timed out, byte-identical bytes re-written). **Presentation semantics ride in the payload** so you render chip colour/severity from data, not a hardcoded class map — each class carries **`fault`** ∈ `self` (our transform bug — render loud) \| `content` (an injected-context byte changed — fixable by a peel/strip) \| `environment` (cache lapsed — fixable by keep-warm) plus a human **`fix_hint`**. The `/_bust?session=` endpoint is the byte-level drill-down; `/_session?...&turn=` deep-links a specific bust. **Supersedes the old `cold_resumes` scalar** (which counted only the `lapse` class and missed every warm content-divergence bust — e.g. a midnight `currentDate` rollover); `lapse` now lives in `busts.by_class.lapse`. **Survives proxy restarts** — this is your source of truth for reconciliation. |
