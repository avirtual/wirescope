...
| **Pull** | `/_bust?session=<id>` | GET | cheap (one disk scan) | **Where did the cache bust?** Disk-based cache-divergence forensics over the session's main line, chronological. Walks adjacent request pairs and, per transition, decides from the RECEIPT whether a rewrite happened + how big (`severity: append`\|`partial`\|`full-rewrite`, `write_frac`, `quasi_full_rewrite`), then localizes it two ways: **`survived_prefix`** = the deepest cache boundary the receipt's `cache_read` covered (`{boundary:"tools"\|"system"\|"messages[i]"\|"none", est_tokens}` — the bill's own answer to how deep the cache held) and **`locus`** = the first byte-change vs the previous forwarded turn (`{segment:"tools"\|"system"\|"messages", index, label, char_offset, old, new}` — the human-readable *what* changed: a model-flavor swap, a `# currentDate` rollover, a transform toggle). **`static_prefix_bust:true`** flags the actionable class — the cached preamble (tools / system / `messages[0]` claudeMd bundle) changed, distinct from a routine cold-resume history rewrite. Payload: `{count, n_busts, n_static_prefix_busts, worst, static_prefix_busts:[…], busts:[…], transitions:[…]}`. Gate on `capabilities.bust_locator`. The `/_session?...&turn=` panel is the per-turn HTML face of this. |
