...
| **Pull** | `/_context?session=<id>` | GET | free | The **tool roster loaded for a session** — what tools are in context right now, per agent line. Returns `agents:[]`, each `{line:"main"\|"subagent", role, agent_id, display_name, model, wire, tools}` where `tools = {count, names, total_schema_chars, est_tokens, per_tool:[{name, schema_chars, est_tokens}]}` (`per_tool` biggest-first — the "what to trim" view). Rosters are the **actually-forwarded** sets (post-wirescope-trim), so the main agent and each subagent show their distinct tool sets. In-memory only: a cold/ended session with nothing live returns `agents:[]` + a `note`. Codex/openai-wire entries report `wire:"openai"`, `tools:null`. |
