...
def _identity():
    ...
    return {
        ...
        "capabilities": {
            ...
            # L2/L3 are NOT separate capabilities: they're levels of THIS mechanism.
            # L1 = prior-turn thinking only; L2 = L1 PLUS the bust-riding
            # tool-result strips (edit-ack collapse + failed-call stubbing); L3 =
            # L2 PLUS read+edit FOLD (apply same-turn edits onto the Read buffer).
            # Each level contains the lower. Gate a level on `max_level >= N`, NOT
            # a per-feature key. Set per session via /_strip?level=N or
            # [wirescope:strip-thinking l2|l3].
